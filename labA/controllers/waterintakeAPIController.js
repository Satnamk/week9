...
const getWaterintake = async (req, res) => {
  try {
    ...
    if (!waterintake) {
      return res.status(404).json({ waterintake: "Waterintake not found" });
    }
...
const deleteWaterintake = async (req, res) => {
  try {
    ...
    res.status(200).json({ waterintake: "Waterintake deleted successfully" });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }
...
const deleteAllWaterintakes = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ waterintake: `Deleted ${result.deletedCount} books successfully` });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }

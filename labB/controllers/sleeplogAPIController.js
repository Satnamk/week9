...
const getSleeplog = async (req, res) => {
  try {
    ...
    if (!sleeplog) {
      return res.status(404).json({ sleeplog: "Sleeplog not found" });
    }
...
const deleteSleeplog = async (req, res) => {
  try {
    ...
    res.status(200).json({ sleeplog: "Sleeplog deleted successfully" });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }
...
const deleteAllSleeplogs = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ sleeplog: `Deleted ${result.deletedCount} books successfully` });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }

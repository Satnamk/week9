...
const getPayment = async (req, res) => {
  try {
    ...
    if (!payment) {
      return res.status(404).json({ payment: "Payment not found" });
    }
...
const deletePayment = async (req, res) => {
  try {
    ...
    res.status(200).json({ payment: "Payment deleted successfully" });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }
...
const deleteAllPayments = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ payment: `Deleted ${result.deletedCount} books successfully` });
  } catch (error) {
    console.error("Error rendering index.html:", error);
    res.status(500).send("Internal Server Error");
  }
